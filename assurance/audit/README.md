...
2. Use a separate, strictly clean checkout at that exact subject to generate
   `assurance/audit/freezes/dcrypt-v3.0.0-audit-candidate-003/`.
...
Candidate `003` directly supersedes candidate `002`; the canonical ordered
history is candidate `001`, then candidate `002`. Candidate `001` has the
exact SOW status
`invalidated-after-partial-independent-replay-before-acceptance`: candidate
`002` superseded it because a documented PTY wrapper mismatch invalidated it
after partial independent replay observations but before completion and
acceptance under the full replay contract. Candidate `001` bound subject
`de9f8a118c227643a938251ddae3d8bc426aaa98` and evidence commit
`d8c7949a94c53cda00efdff99ff6b55413a5e1a6`; those partial observations and
generated bytes remain diagnostic and unaccepted.

Candidate `002` bound subject
`852c771c7d764752e23322ab412b419925fb5a5f`, tree
`ae5779e73b64b60cc4ce198468ef9fd781cda2df`, and parent/SOW commit
`8e3b8d7ee3ea9ec7d1901dadf9c85f3aa0706c02`. Its exact SOW status is
`invalidated-after-first-party-diagnostic-before-valid-wrapper-compliant-generation`.
It produced a first-party diagnostic temporary materialization and diagnostic
13-file bundle, but no valid wrapper-compliant materialization, valid candidate
generation, evidence commit, complete independent replay, or acceptance. The
ordered exhaustive known-defect set is:

1. `provision/generation README fences could return 0 after failed gate due trailing assignment`;
...
Candidate `002`'s recorded hashes are typed first-party diagnostic non-evidence.
The independent review scope was source, self-test, and fence diagnostics only,
not candidate-bundle evidence. Neither superseded candidate produced accepted
assurance/audit evidence, and candidate `002` involved no external audit or
vendor contact. The freeze history is derived from, and required to equal, the
committed SOW policy and scope; A2 stores only their canonical history pointer
and digest. None of the superseded bytes count as evidence for candidate `003`.
...
procedure. Set `AUDIT_SOURCE`, an existing private `AUDIT_CHECKOUT_PARENT`, and
the already reviewed exact 40-hex `AUDIT_SUBJECT`; leave `AUDIT_CHECKOUT`
unset. The block promotes `AUDIT_CHECKOUT` only after both clone and checkout
succeed. A failed attempt may leave its uniquely named diagnostic staging tree,
but never promotes that tree for later use. Run with this closed environment;
...
```sh
AUDIT_CHECKOUT_PREPARE_RC=0
unset AUDIT_CHECKOUT
if AUDIT_CHECKOUT_STAGE_PARENT=$(/usr/bin/mktemp -d \
  "$AUDIT_CHECKOUT_PARENT/dcrypt-audit-checkout.XXXXXX") &&
AUDIT_CHECKOUT_STAGE="$AUDIT_CHECKOUT_STAGE_PARENT/checkout" &&
(
  umask 0022 &&
  env -i LANG=C.UTF-8 LC_ALL=C.UTF-8 TZ=UTC GIT_ALLOW_PROTOCOL=file \
    ...
    --no-hardlinks --no-checkout "$AUDIT_SOURCE" "$AUDIT_CHECKOUT_STAGE" &&
  env -i LANG=C.UTF-8 LC_ALL=C.UTF-8 TZ=UTC GIT_ALLOW_PROTOCOL=file \
    GIT_ASKPASS=/bin/false GIT_ATTR_NOSYSTEM=1 GIT_CONFIG_COUNT=0 \
    GIT_CONFIG_GLOBAL=/dev/null GIT_CONFIG_NOSYSTEM=1 \
    GIT_CONFIG_SYSTEM=/dev/null GIT_LFS_SKIP_SMUDGE=1 \
    ...
    GIT_PAGER=cat GIT_PROTOCOL_FROM_USER=0 GIT_TERMINAL_PROMPT=0 \
    PAGER=cat SSH_ASKPASS=/bin/false \
    /usr/bin/git -C "$AUDIT_CHECKOUT_STAGE" checkout --detach "$AUDIT_SUBJECT"
)
then
  AUDIT_CHECKOUT="$AUDIT_CHECKOUT_STAGE_PARENT/checkout"
  AUDIT_CHECKOUT_PREPARE_RC=0
else
  AUDIT_CHECKOUT_PREPARE_RC=$?
  unset AUDIT_CHECKOUT
fi
...
```sh
PROVISION_BLOCK_RC=0
unset PROVISION_HANDOFF
if PROVISION_OUTPUT=$(/usr/bin/mktemp -d /dev/shm/dcrypt-audit-output.XXXXXX) &&
(
PROVISION_LOG_DIR=$(/usr/bin/mktemp -d /dev/shm/dcrypt-audit-provision.XXXXXX) || exit $?
...
set +C
umask 0022 || exit $?
exec 9</proc/self/fd/8
PROVISION_LOG_ID=$(/usr/bin/stat -Lc %d:%i /proc/self/fd/8)
...
test -f /proc/self/fd/8 && test -f /proc/self/fd/9 || PROVISION_LOG_PRECHECK=1
test "$(/usr/bin/stat -Lc %d:%i /proc/self/fd/9)" = "$PROVISION_LOG_ID" || PROVISION_LOG_PRECHECK=1
test "$(/usr/bin/stat -Lc %h /proc/self/fd/8)" = 1 || PROVISION_LOG_PRECHECK=1
test "$(/usr/bin/stat -Lc %a /proc/self/fd/8)" = 600 || PROVISION_LOG_PRECHECK=1
test "$(/usr/bin/stat -Lc %s /proc/self/fd/8)" = 0 || PROVISION_LOG_PRECHECK=1
...
test -f /proc/self/fd/8 && test -f /proc/self/fd/9 || PROVISION_LOG_POSTCHECK=1
test "$(/usr/bin/stat -Lc %d:%i /proc/self/fd/8)" = "$PROVISION_LOG_ID" || PROVISION_LOG_POSTCHECK=1
test "$(/usr/bin/stat -Lc %d:%i /proc/self/fd/9)" = "$PROVISION_LOG_ID" || PROVISION_LOG_POSTCHECK=1
test "$(/usr/bin/stat -Lc %h /proc/self/fd/8)" = 1 || PROVISION_LOG_POSTCHECK=1
test "$(/usr/bin/stat -Lc %a /proc/self/fd/8)" = 600 || PROVISION_LOG_POSTCHECK=1
test "$(/usr/bin/stat -Lc %s /proc/self/fd/8)" -le 16777216 || PROVISION_LOG_POSTCHECK=1
...
then
  PROVISION_LOG_SNAPSHOT=$(/usr/bin/stat -Lc '%d:%i:%f:%h:%s:%y:%z' /proc/self/fd/8) || PROVISION_LOG_POSTCHECK=1
  PROVISION_LOG_SIZE=$(/usr/bin/stat -Lc %s /proc/self/fd/8) || PROVISION_LOG_POSTCHECK=1
fi
if test "$PROVISION_LOG_POSTCHECK" -eq 0
then
  test "$(/usr/bin/stat -Lc '%d:%i:%f:%h:%s:%y:%z' /proc/self/fd/9)" = "$PROVISION_LOG_SNAPSHOT" || PROVISION_LOG_POSTCHECK=1
  test "$(/usr/bin/stat -c '%d:%i:%f:%h:%s:%y:%z' "$PROVISION_LOG")" = "$PROVISION_LOG_SNAPSHOT" || PROVISION_LOG_POSTCHECK=1
  PROVISION_LOG_FINAL=$(/usr/bin/stat -Lc '%d:%i:%f:%h:%s:%y:%z' /proc/self/fd/8) || PROVISION_LOG_POSTCHECK=1
  test "$PROVISION_LOG_FINAL" = "$PROVISION_LOG_SNAPSHOT" || PROVISION_LOG_POSTCHECK=1
  test "$(/usr/bin/stat -Lc %s /proc/self/fd/8)" = "$PROVISION_LOG_SIZE" || PROVISION_LOG_POSTCHECK=1
  test "$(/usr/bin/stat -Lc '%d:%i:%f:%h:%s:%y:%z' /proc/self/fd/9)" = "$PROVISION_LOG_SNAPSHOT" || PROVISION_LOG_POSTCHECK=1
  test "$(/usr/bin/stat -c '%d:%i:%f:%h:%s:%y:%z' "$PROVISION_LOG")" = "$PROVISION_LOG_SNAPSHOT" || PROVISION_LOG_POSTCHECK=1
fi
exec 9<&-
...
PROVISION_OUTPUT_POSTCHECK=1
if test "$PROVISION_RC" -eq 0 &&
  env -i LANG=C.UTF-8 LC_ALL=C.UTF-8 TZ=UTC \
    /usr/bin/python3.12 -I -B -S \
    "$AUDIT_CHECKOUT/assurance/generate-audit-freeze.py" \
    --verify-exact provisioning \
    --source "$PROVISION_OUTPUT/dcrypt-audit-provisioning-v1"
then
  PROVISION_OUTPUT_POSTCHECK=0
fi
test "$PROVISION_LOG_PRECHECK" -eq 0 && \
  test "$PROVISION_LOG_POSTCHECK" -eq 0 && \
  test "$PROVISION_OUTPUT_POSTCHECK" -eq 0 && \
...
```sh
GENERATION_BLOCK_RC=0
unset CANDIDATE
if GENERATION_OUTPUT=$(/usr/bin/mktemp -d /dev/shm/dcrypt-audit-output.XXXXXX) &&
(
GENERATION_LOG_DIR=$(/usr/bin/mktemp -d /dev/shm/dcrypt-audit-generation.XXXXXX) || exit $?
...
umask 0022 || exit $?
exec 9</proc/self/fd/8
GENERATION_LOG_ID=$(/usr/bin/stat -Lc %d:%i /proc/self/fd/8)
...
test -f /proc/self/fd/8 && test -f /proc/self/fd/9 || GENERATION_LOG_PRECHECK=1
test "$(/usr/bin/stat -Lc %d:%i /proc/self/fd/9)" = "$GENERATION_LOG_ID" || GENERATION_LOG_PRECHECK=1
test "$(/usr/bin/stat -Lc %h /proc/self/fd/8)" = 1 || GENERATION_LOG_PRECHECK=1
test "$(/usr/bin/stat -Lc %a /proc/self/fd/8)" = 600 || GENERATION_LOG_PRECHECK=1
test "$(/usr/bin/stat -Lc %s /proc/self/fd/8)" = 0 || GENERATION_LOG_PRECHECK=1
...
then
  if (
    ...
    /usr/bin/bwrap \
      ...
      --output /output/dcrypt-v3.0.0-audit-candidate-003
  ) </dev/null >&8 2>&1
  then
    GENERATION_RC=0
  else
...
test -f /proc/self/fd/8 && test -f /proc/self/fd/9 || GENERATION_LOG_POSTCHECK=1
test "$(/usr/bin/stat -Lc %d:%i /proc/self/fd/8)" = "$GENERATION_LOG_ID" || GENERATION_LOG_POSTCHECK=1
test "$(/usr/bin/stat -Lc %d:%i /proc/self/fd/9)" = "$GENERATION_LOG_ID" || GENERATION_LOG_POSTCHECK=1
test "$(/usr/bin/stat -Lc %h /proc/self/fd/8)" = 1 || GENERATION_LOG_POSTCHECK=1
test "$(/usr/bin/stat -Lc %a /proc/self/fd/8)" = 600 || GENERATION_LOG_POSTCHECK=1
test "$(/usr/bin/stat -Lc %s /proc/self/fd/8)" -le 16777216 || GENERATION_LOG_POSTCHECK=1
...
then
  GENERATION_LOG_SNAPSHOT=$(/usr/bin/stat -Lc '%d:%i:%f:%h:%s:%y:%z' /proc/self/fd/8) || GENERATION_LOG_POSTCHECK=1
  GENERATION_LOG_SIZE=$(/usr/bin/stat -Lc %s /proc/self/fd/8) || GENERATION_LOG_POSTCHECK=1
fi
if test "$GENERATION_LOG_POSTCHECK" -eq 0
then
  test "$(/usr/bin/stat -Lc '%d:%i:%f:%h:%s:%y:%z' /proc/self/fd/9)" = "$GENERATION_LOG_SNAPSHOT" || GENERATION_LOG_POSTCHECK=1
  test "$(/usr/bin/stat -c '%d:%i:%f:%h:%s:%y:%z' "$GENERATION_LOG")" = "$GENERATION_LOG_SNAPSHOT" || GENERATION_LOG_POSTCHECK=1
  GENERATION_LOG_FINAL=$(/usr/bin/stat -Lc '%d:%i:%f:%h:%s:%y:%z' /proc/self/fd/8) || GENERATION_LOG_POSTCHECK=1
  test "$GENERATION_LOG_FINAL" = "$GENERATION_LOG_SNAPSHOT" || GENERATION_LOG_POSTCHECK=1
  test "$(/usr/bin/stat -Lc %s /proc/self/fd/8)" = "$GENERATION_LOG_SIZE" || GENERATION_LOG_POSTCHECK=1
  test "$(/usr/bin/stat -Lc '%d:%i:%f:%h:%s:%y:%z' /proc/self/fd/9)" = "$GENERATION_LOG_SNAPSHOT" || GENERATION_LOG_POSTCHECK=1
  test "$(/usr/bin/stat -c '%d:%i:%f:%h:%s:%y:%z' "$GENERATION_LOG")" = "$GENERATION_LOG_SNAPSHOT" || GENERATION_LOG_POSTCHECK=1
fi
exec 9<&-
...
GENERATION_OUTPUT_POSTCHECK=1
if test "$GENERATION_RC" -eq 0 &&
  env -i LANG=C.UTF-8 LC_ALL=C.UTF-8 TZ=UTC \
    /usr/bin/python3.12 -I -B -S \
    "$AUDIT_CHECKOUT/assurance/generate-audit-freeze.py" \
    --verify-exact candidate \
    --source "$GENERATION_OUTPUT/dcrypt-v3.0.0-audit-candidate-003"
then
  GENERATION_OUTPUT_POSTCHECK=0
fi
test "$GENERATION_LOG_PRECHECK" -eq 0 && \
  test "$GENERATION_LOG_POSTCHECK" -eq 0 && \
  test "$GENERATION_OUTPUT_POSTCHECK" -eq 0 && \
  test "$GENERATION_RC" -eq 0
)
then
  CANDIDATE="$GENERATION_OUTPUT/dcrypt-v3.0.0-audit-candidate-003"
  GENERATION_BLOCK_RC=0
else
  GENERATION_BLOCK_RC=$?
  unset CANDIDATE
...
To copy into the evidence checkout without preserving links, use the exact
transfer mode below. It descriptor-holds and rechecks all 13 expected source
files, their modes, link counts, sizes, checksums, and complete directory
inventory, then uses the same atomic writer as generation. Missing, extra,
symlinked, hardlinked, special, changing, malformed-JSON, noncanonical-JSON, or
schema-invalid input fails before a destination directory is promoted. Every
candidate JSON document and the provisioning manifest is canonical-parsed;
the bound freeze, envelope, and provisioning schemas and cross-document
digests are enforced even when an attacker has coherently recomputed the
checksum files. Do not use `cp -a`, reflinks, hardlinks, or a recursive archive
extraction. Commit only that exact destination. The verifier still compares
worktree files with committed Git blobs, so this first-party handoff remains
untrusted.

```sh
EVIDENCE_TRANSFER_RC=0
unset EVIDENCE_BUNDLE
EVIDENCE_BUNDLE_TARGET="$EVIDENCE_CHECKOUT/assurance/audit/freezes/dcrypt-v3.0.0-audit-candidate-003"
if (
  umask 0022 &&
  /usr/bin/mkdir -p "$(/usr/bin/dirname "$EVIDENCE_BUNDLE_TARGET")" &&
  env -i LANG=C.UTF-8 LC_ALL=C.UTF-8 TZ=UTC \
    /usr/bin/python3.12 -I -B -S \
    "$AUDIT_CHECKOUT/assurance/generate-audit-freeze.py" \
    --transfer-exact candidate --source "$CANDIDATE" \
    --output "$EVIDENCE_BUNDLE_TARGET"
)
then
  EVIDENCE_BUNDLE="$EVIDENCE_BUNDLE_TARGET"
  EVIDENCE_TRANSFER_RC=0
else
  EVIDENCE_TRANSFER_RC=$?
  unset EVIDENCE_BUNDLE
fi
...
to a fresh tmpfs directory through the same descriptor-safe exact transfer
mode; it writes fresh uniquely linked files and does not preserve source
metadata or links. Bind that directory as `/provision` and run:

```sh
PROVISION_TRANSFER_RC=0
REPLAY_PROVISION_SOURCE="$PROVISION_HANDOFF"
unset PROVISION_HANDOFF
if REPLAY_PROVISION_PARENT=$(/usr/bin/mktemp -d /dev/shm/dcrypt-audit-handoff.XXXXXX) &&
(
  umask 0022 &&
  env -i LANG=C.UTF-8 LC_ALL=C.UTF-8 TZ=UTC \
    /usr/bin/python3.12 -I -B -S \
    "$SUBJECT_CHECKOUT/assurance/generate-audit-freeze.py" \
    --transfer-exact provisioning --source "$REPLAY_PROVISION_SOURCE" \
    --output "$REPLAY_PROVISION_PARENT/dcrypt-audit-provisioning-v1"
)
then
  PROVISION_HANDOFF="$REPLAY_PROVISION_PARENT/dcrypt-audit-provisioning-v1"
  PROVISION_TRANSFER_RC=0
else
  PROVISION_TRANSFER_RC=$?
  unset PROVISION_HANDOFF
fi
...
```sh
STRUCTURAL_BLOCK_RC=0
if (
STRUCTURAL_LOG_DIR=$(/usr/bin/mktemp -d /dev/shm/dcrypt-audit-structural.XXXXXX) || exit $?
...
umask 0022 || exit $?
exec 9</proc/self/fd/8
STRUCTURAL_LOG_ID=$(/usr/bin/stat -Lc %d:%i /proc/self/fd/8)
...
test -f /proc/self/fd/8 && test -f /proc/self/fd/9 || STRUCTURAL_LOG_PRECHECK=1
test "$(/usr/bin/stat -Lc %d:%i /proc/self/fd/9)" = "$STRUCTURAL_LOG_ID" || STRUCTURAL_LOG_PRECHECK=1
test "$(/usr/bin/stat -Lc %h /proc/self/fd/8)" = 1 || STRUCTURAL_LOG_PRECHECK=1
test "$(/usr/bin/stat -Lc %a /proc/self/fd/8)" = 600 || STRUCTURAL_LOG_PRECHECK=1
test "$(/usr/bin/stat -Lc %s /proc/self/fd/8)" = 0 || STRUCTURAL_LOG_PRECHECK=1
...
then
  if (
    ...
    /usr/bin/bwrap \
      ...
      --bundle /evidence/assurance/audit/freezes/dcrypt-v3.0.0-audit-candidate-003 \
      --mode structural
  ) </dev/null >&8 2>&1
  then
    STRUCTURAL_RC=0
...
test -f /proc/self/fd/8 && test -f /proc/self/fd/9 || STRUCTURAL_LOG_POSTCHECK=1
test "$(/usr/bin/stat -Lc %d:%i /proc/self/fd/8)" = "$STRUCTURAL_LOG_ID" || STRUCTURAL_LOG_POSTCHECK=1
test "$(/usr/bin/stat -Lc %d:%i /proc/self/fd/9)" = "$STRUCTURAL_LOG_ID" || STRUCTURAL_LOG_POSTCHECK=1
test "$(/usr/bin/stat -Lc %h /proc/self/fd/8)" = 1 || STRUCTURAL_LOG_POSTCHECK=1
test "$(/usr/bin/stat -Lc %a /proc/self/fd/8)" = 600 || STRUCTURAL_LOG_POSTCHECK=1
test "$(/usr/bin/stat -Lc %s /proc/self/fd/8)" -le 16777216 || STRUCTURAL_LOG_POSTCHECK=1
...
then
  STRUCTURAL_LOG_SNAPSHOT=$(/usr/bin/stat -Lc '%d:%i:%f:%h:%s:%y:%z' /proc/self/fd/8) || STRUCTURAL_LOG_POSTCHECK=1
  STRUCTURAL_LOG_SIZE=$(/usr/bin/stat -Lc %s /proc/self/fd/8) || STRUCTURAL_LOG_POSTCHECK=1
fi
if test "$STRUCTURAL_LOG_POSTCHECK" -eq 0
then
  test "$(/usr/bin/stat -Lc '%d:%i:%f:%h:%s:%y:%z' /proc/self/fd/9)" = "$STRUCTURAL_LOG_SNAPSHOT" || STRUCTURAL_LOG_POSTCHECK=1
  test "$(/usr/bin/stat -c '%d:%i:%f:%h:%s:%y:%z' "$STRUCTURAL_LOG")" = "$STRUCTURAL_LOG_SNAPSHOT" || STRUCTURAL_LOG_POSTCHECK=1
  STRUCTURAL_LOG_FINAL=$(/usr/bin/stat -Lc '%d:%i:%f:%h:%s:%y:%z' /proc/self/fd/8) || STRUCTURAL_LOG_POSTCHECK=1
  test "$STRUCTURAL_LOG_FINAL" = "$STRUCTURAL_LOG_SNAPSHOT" || STRUCTURAL_LOG_POSTCHECK=1
  test "$(/usr/bin/stat -Lc %s /proc/self/fd/8)" = "$STRUCTURAL_LOG_SIZE" || STRUCTURAL_LOG_POSTCHECK=1
  test "$(/usr/bin/stat -Lc '%d:%i:%f:%h:%s:%y:%z' /proc/self/fd/9)" = "$STRUCTURAL_LOG_SNAPSHOT" || STRUCTURAL_LOG_POSTCHECK=1
  test "$(/usr/bin/stat -c '%d:%i:%f:%h:%s:%y:%z' "$STRUCTURAL_LOG")" = "$STRUCTURAL_LOG_SNAPSHOT" || STRUCTURAL_LOG_POSTCHECK=1
fi
exec 9<&-
...
RELEASE_BLOCK_RC=0
if (
RELEASE_LOG_DIR=$(/usr/bin/mktemp -d /dev/shm/dcrypt-audit-release.XXXXXX) || exit $?
...
umask 0022 || exit $?
exec 9</proc/self/fd/8
RELEASE_LOG_ID=$(/usr/bin/stat -Lc %d:%i /proc/self/fd/8)
...
test -f /proc/self/fd/8 && test -f /proc/self/fd/9 || RELEASE_LOG_PRECHECK=1
test "$(/usr/bin/stat -Lc %d:%i /proc/self/fd/9)" = "$RELEASE_LOG_ID" || RELEASE_LOG_PRECHECK=1
test "$(/usr/bin/stat -Lc %h /proc/self/fd/8)" = 1 || RELEASE_LOG_PRECHECK=1
test "$(/usr/bin/stat -Lc %a /proc/self/fd/8)" = 600 || RELEASE_LOG_PRECHECK=1
test "$(/usr/bin/stat -Lc %s /proc/self/fd/8)" = 0 || RELEASE_LOG_PRECHECK=1
...
test -f /proc/self/fd/8 && test -f /proc/self/fd/9 || RELEASE_LOG_POSTCHECK=1
test "$(/usr/bin/stat -Lc %d:%i /proc/self/fd/8)" = "$RELEASE_LOG_ID" || RELEASE_LOG_POSTCHECK=1
test "$(/usr/bin/stat -Lc %d:%i /proc/self/fd/9)" = "$RELEASE_LOG_ID" || RELEASE_LOG_POSTCHECK=1
test "$(/usr/bin/stat -Lc %h /proc/self/fd/8)" = 1 || RELEASE_LOG_POSTCHECK=1
test "$(/usr/bin/stat -Lc %a /proc/self/fd/8)" = 600 || RELEASE_LOG_POSTCHECK=1
test "$(/usr/bin/stat -Lc %s /proc/self/fd/8)" -le 16777216 || RELEASE_LOG_POSTCHECK=1
...
then
  RELEASE_LOG_SNAPSHOT=$(/usr/bin/stat -Lc '%d:%i:%f:%h:%s:%y:%z' /proc/self/fd/8) || RELEASE_LOG_POSTCHECK=1
  RELEASE_LOG_SIZE=$(/usr/bin/stat -Lc %s /proc/self/fd/8) || RELEASE_LOG_POSTCHECK=1
fi
if test "$RELEASE_LOG_POSTCHECK" -eq 0
then
  test "$(/usr/bin/stat -Lc '%d:%i:%f:%h:%s:%y:%z' /proc/self/fd/9)" = "$RELEASE_LOG_SNAPSHOT" || RELEASE_LOG_POSTCHECK=1
  test "$(/usr/bin/stat -c '%d:%i:%f:%h:%s:%y:%z' "$RELEASE_LOG")" = "$RELEASE_LOG_SNAPSHOT" || RELEASE_LOG_POSTCHECK=1
  RELEASE_LOG_FINAL=$(/usr/bin/stat -Lc '%d:%i:%f:%h:%s:%y:%z' /proc/self/fd/8) || RELEASE_LOG_POSTCHECK=1
  test "$RELEASE_LOG_FINAL" = "$RELEASE_LOG_SNAPSHOT" || RELEASE_LOG_POSTCHECK=1
  test "$(/usr/bin/stat -Lc %s /proc/self/fd/8)" = "$RELEASE_LOG_SIZE" || RELEASE_LOG_POSTCHECK=1
  test "$(/usr/bin/stat -Lc '%d:%i:%f:%h:%s:%y:%z' /proc/self/fd/9)" = "$RELEASE_LOG_SNAPSHOT" || RELEASE_LOG_POSTCHECK=1
  test "$(/usr/bin/stat -c '%d:%i:%f:%h:%s:%y:%z' "$RELEASE_LOG")" = "$RELEASE_LOG_SNAPSHOT" || RELEASE_LOG_POSTCHECK=1
fi
exec 9<&-
...
SELFTEST_BLOCK_RC=0
if (
SELFTEST_OUTPUT=$(/usr/bin/mktemp -d /dev/shm/dcrypt-audit-output.XXXXXX) || exit $?
SELFTEST_LOG_DIR=$(/usr/bin/mktemp -d /dev/shm/dcrypt-audit-selftest.XXXXXX) || exit $?
...
umask 0022 || exit $?
exec 9</proc/self/fd/8
SELFTEST_LOG_ID=$(/usr/bin/stat -Lc %d:%i /proc/self/fd/8)
...
test -f /proc/self/fd/8 && test -f /proc/self/fd/9 || SELFTEST_LOG_PRECHECK=1
test "$(/usr/bin/stat -Lc %d:%i /proc/self/fd/9)" = "$SELFTEST_LOG_ID" || SELFTEST_LOG_PRECHECK=1
test "$(/usr/bin/stat -Lc %h /proc/self/fd/8)" = 1 || SELFTEST_LOG_PRECHECK=1
test "$(/usr/bin/stat -Lc %a /proc/self/fd/8)" = 600 || SELFTEST_LOG_PRECHECK=1
test "$(/usr/bin/stat -Lc %s /proc/self/fd/8)" = 0 || SELFTEST_LOG_PRECHECK=1
...
test -f /proc/self/fd/8 && test -f /proc/self/fd/9 || SELFTEST_LOG_POSTCHECK=1
test "$(/usr/bin/stat -Lc %d:%i /proc/self/fd/8)" = "$SELFTEST_LOG_ID" || SELFTEST_LOG_POSTCHECK=1
test "$(/usr/bin/stat -Lc %d:%i /proc/self/fd/9)" = "$SELFTEST_LOG_ID" || SELFTEST_LOG_POSTCHECK=1
test "$(/usr/bin/stat -Lc %h /proc/self/fd/8)" = 1 || SELFTEST_LOG_POSTCHECK=1
test "$(/usr/bin/stat -Lc %a /proc/self/fd/8)" = 600 || SELFTEST_LOG_POSTCHECK=1
test "$(/usr/bin/stat -Lc %s /proc/self/fd/8)" -le 16777216 || SELFTEST_LOG_POSTCHECK=1
...
then
  SELFTEST_LOG_SNAPSHOT=$(/usr/bin/stat -Lc '%d:%i:%f:%h:%s:%y:%z' /proc/self/fd/8) || SELFTEST_LOG_POSTCHECK=1
  SELFTEST_LOG_SIZE=$(/usr/bin/stat -Lc %s /proc/self/fd/8) || SELFTEST_LOG_POSTCHECK=1
fi
if test "$SELFTEST_LOG_POSTCHECK" -eq 0
then
  test "$(/usr/bin/stat -Lc '%d:%i:%f:%h:%s:%y:%z' /proc/self/fd/9)" = "$SELFTEST_LOG_SNAPSHOT" || SELFTEST_LOG_POSTCHECK=1
  test "$(/usr/bin/stat -c '%d:%i:%f:%h:%s:%y:%z' "$SELFTEST_LOG")" = "$SELFTEST_LOG_SNAPSHOT" || SELFTEST_LOG_POSTCHECK=1
  SELFTEST_LOG_FINAL=$(/usr/bin/stat -Lc '%d:%i:%f:%h:%s:%y:%z' /proc/self/fd/8) || SELFTEST_LOG_POSTCHECK=1
  test "$SELFTEST_LOG_FINAL" = "$SELFTEST_LOG_SNAPSHOT" || SELFTEST_LOG_POSTCHECK=1
  test "$(/usr/bin/stat -Lc %s /proc/self/fd/8)" = "$SELFTEST_LOG_SIZE" || SELFTEST_LOG_POSTCHECK=1
  test "$(/usr/bin/stat -Lc '%d:%i:%f:%h:%s:%y:%z' /proc/self/fd/9)" = "$SELFTEST_LOG_SNAPSHOT" || SELFTEST_LOG_POSTCHECK=1
  test "$(/usr/bin/stat -c '%d:%i:%f:%h:%s:%y:%z' "$SELFTEST_LOG")" = "$SELFTEST_LOG_SNAPSHOT" || SELFTEST_LOG_POSTCHECK=1
fi
exec 9<&-
