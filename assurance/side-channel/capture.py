#!/usr/bin/env python3
"""Descriptor-anchored import of an already-produced private Package D bundle.
...
def _os_error(label: str, error: OSError) -> PackageDError:
    """Return an error which cannot disclose a private path or candidate value."""

    return PackageDError(f"{label} failed with operating-system errno {error.errno}")
...
def _canonical_absolute(path: Path, *, label: str) -> Path:
    ...
    if (
        ...
        raise PackageDError(f"{label} must be a canonical absolute path without .git")
    return path
...
def _open_directory_chain(
    ...
    try:
        ...
        if not stat.S_ISDIR(metadata.st_mode):
            raise PackageDError(f"{label} root is not a directory")
        ...
        for component in path.parts[1:]:
            ...
            metadata = os.fstat(descriptor)
            if not stat.S_ISDIR(metadata.st_mode):
                raise PackageDError(f"{label} contains a nondirectory component")
            identities.append(_basic_identity(metadata))
...
def _assert_directory_chain(
    ...
    try:
        if observed != expected:
            raise PackageDError(f"{label} identity changed during capture")
    finally:
        os.close(descriptor)
...
def _entry_absent(parent_fd: int, name: str) -> None:
    ...
    raise PackageDError("destination must not already exist")
        ...
        with os.scandir(descriptor) as entries:
            for entry in entries:
                ...
                if len(names) > expected_count:
                    raise PackageDError(f"{label} contains a surplus entry")
    except OSError as error:
...
def _validate_roots(
    ...
    if (
        ...
        raise PackageDError("source or destination overlaps a forbidden boundary")
    ...
    try:
        ...
        if not _private_directory(source_metadata):
            raise PackageDError("source bundle root is not current-owner mode 0700")
        ...
        if not _private_directory(parent_metadata):
            raise PackageDError("destination parent is not current-owner mode 0700")
        ...
        if (
            ...
            raise PackageDError("descriptor identities overlap a forbidden boundary")
...
def _read_descriptor(
    ...
    try:
        while remaining:
            ...
            if not chunk:
                raise PackageDError(f"{label} was truncated during capture")
            chunks.append(chunk)
            remaining -= len(chunk)
        if os.read(descriptor, 1) != b"":
            raise PackageDError(f"{label} grew during capture")
        if _stable_identity(os.fstat(descriptor)) != expected:
            raise PackageDError(f"{label} metadata changed during capture")
    except OSError as error:
...
def _open_candidate(root_fd: int) -> tuple[int, os.stat_result]:
    ...
    try:
        ...
        if not _private_file(metadata, MAX_CANDIDATE_BYTES):
            raise PackageDError("candidate record is not a bounded private regular file")
...
def _candidate_contract(
    ...
    if role in {"acceptance", "physical"}:
        raise PackageDError("Package D v1 rejects this capture role")
    if role not in ROLE_CAPS or role not in ROLE_PROFILES:
        raise PackageDError("capture role has no code-pinned resource profile")
    caps = ROLE_CAPS[role]
    if set(caps) != {"files", "per_file", "total"} or any(
        not isinstance(value, int) or isinstance(value, bool) or value <= 0
        for value in caps.values()
    ):
        raise PackageDError("code-pinned role cap closure differs")
    profile_id = candidate["profile_id"]
    if profile_id not in ROLE_PROFILES[role]:
        raise PackageDError("candidate profile is not reviewed for its role")
    if candidate["binding"]["profile_sha256"] != profile_sha256(role, profile_id):
        raise PackageDError("candidate profile digest differs")
    if candidate["binding"]["source_set_sha256"] != EXPECTED_SOURCE_ROWS_SHA256:
        raise PackageDError("candidate source-set binding differs")

    artifacts = candidate["artifacts"]
    if len(artifacts) > caps["files"]:
        raise PackageDError("candidate exceeds its role-specific file cap")
    expected_digest = artifact_set_sha256(artifacts)
    if candidate["raw_artifact_set_sha256"] != expected_digest:
        raise PackageDError("candidate raw-artifact set digest differs")

    records: dict[str, dict[str, Any]] = {}
    expected_files = {"candidate.json"}
    expected_directories: set[str] = set()
    # ROLE_CAPS.total is the raw-artifact total.  candidate.json has its own
    # independent MAX_CANDIDATE_BYTES cap and is not silently charged twice.
    declared_total = 0
    for record in artifacts:
        relative = safe_relative_path(record["path"], label="candidate artifact")
        parts = PurePosixPath(relative).parts
        if ".git" in parts or relative == "candidate.json":
            raise PackageDError("candidate artifact uses a forbidden path")
        if record["size"] > caps["per_file"]:
            raise PackageDError("candidate exceeds its role-specific per-file cap")
    ...
    if declared_total > caps["total"]:
        raise PackageDError("candidate exceeds its role-specific aggregate cap")
    if expected_files & expected_directories:
        raise PackageDError("candidate artifact paths contain a file/directory conflict")
        ...
        if path.startswith(prefix):
            ...
            if "/" not in remainder:
                if children.setdefault(remainder, "directory") != "directory":
                    raise PackageDError("bundle path type closure conflicts")
    return children
...
def _scan_source_tree(
    ...
    def visit(descriptor: int, relative: str) -> None:
        ...
        if set(names) != set(expected):
            raise PackageDError("source bundle path closure differs")
        for name in sorted(names):
            child = f"{relative}/{name}" if relative else name
            if name == ".git":
                raise PackageDError("source bundle contains a forbidden .git component")
            try:
            ...
            if expected[name] == "directory":
                if not _private_directory(metadata):
                    raise PackageDError("source bundle contains an unsafe directory")
                child_fd = -1
                try:
                    child_fd = os.open(
                        name,
                    ...
                    if _stable_identity(opened) != _stable_identity(metadata):
                        raise PackageDError("source directory changed during traversal")
                    key = (opened.st_dev, opened.st_ino)
                    if key in seen_directories:
                        raise PackageDError("source bundle aliases a directory")
                ...
                if not _private_file(metadata, maximum):
                    raise PackageDError("source bundle contains an unsafe file")
                expected_size = (
                    metadata.st_size
                    if child == "candidate.json"
                ...
                if metadata.st_size != expected_size:
                    raise PackageDError("source artifact size differs from its declaration")
    ...
    if set(file_identities) != expected_files or set(directory_identities) != expected_directories:
        raise PackageDError("source bundle identity closure differs")
    ...
    try:
        ...
        for component in parts[:-1]:
            ...
            if _stable_identity(os.fstat(current)) != directory_identities["/".join(prefix)]:
                raise PackageDError("source directory identity changed before read")
        descriptor = os.open(
            parts[-1],
            os.O_RDONLY | os.O_NONBLOCK | os.O_NOFOLLOW,
            dir_fd=current,
        ...
        if _stable_identity(metadata) != file_identities[relative]:
            os.close(descriptor)
            raise PackageDError("source file identity changed before read")
        return descriptor, metadata
    except OSError as error:
...
def _create_staging(parent_fd: int) -> tuple[str, int, tuple[int, ...]]:
    for _attempt in range(32):
        name = f".dcrypt-package-d-capture-{os.urandom(16).hex()}"
        try:
        ...
        try:
            ...
            metadata = os.fstat(descriptor)
            if not _private_directory(metadata):
                raise PackageDError("capture staging directory metadata differs")
            ...
            try:
                ...
                pass
            raise
    raise PackageDError("cannot allocate an exclusive capture staging directory")
        ...
        for component in PurePosixPath(relative).parts[:-1]:
            ...
            if key not in directory_identities:
                ...
                try:
                    ...
                    if not _private_directory(metadata):
                        raise PackageDError("destination directory metadata differs")
                ...
                if _basic_identity(os.fstat(child_fd)) != directory_identities[key]:
                    os.close(child_fd)
                    raise PackageDError("destination directory identity changed")
            os.close(current)
            current = child_fd
        return current
    except OSError as error:
...
def _complete_write(descriptor: int, raw: bytes) -> None:
    ...
    while view:
        ...
        if written <= 0:
            raise PackageDError("destination write made no progress")
        view = view[written:]
...
def _verify_written_descriptor(
    ...
    *,
    ...
    if not _private_file(metadata, expected_size) or metadata.st_size != expected_size:
        raise PackageDError("destination file metadata differs")
    ...
    while remaining:
        chunk = os.read(descriptor, min(1 << 20, remaining))
        if not chunk:
            raise PackageDError("destination file was truncated")
        digest.update(chunk)
        remaining -= len(chunk)
    if os.read(descriptor, 1) != b"" or digest.hexdigest() != expected_sha256:
        raise PackageDError("destination file bytes differ")
    if _stable_identity(os.fstat(descriptor)) != expected_identity:
        raise PackageDError("destination file metadata changed during verification")
...
def _copy_artifact(
    ...
    try:
        ...
        while remaining:
            ...
            if not chunk:
                raise PackageDError("source artifact was truncated during copy")
            digest.update(chunk)
            _complete_write(destination_fd, chunk)
            remaining -= len(chunk)
        if os.read(source_fd, 1) != b"":
            raise PackageDError("source artifact grew during copy")
        if (
            digest.hexdigest() != expected_sha256
            or _stable_identity(os.fstat(source_fd)) != source_identity
        ):
            raise PackageDError("source artifact identity differs from its declaration")
        identity = _verify_written_descriptor(
            destination_fd,
...
def _verify_destination_tree(
    ...
    def visit(descriptor: int, relative: str) -> None:
        ...
        if set(names) != set(wanted):
            raise PackageDError("captured destination closure differs")
        for name in sorted(names):
            child = f"{relative}/{name}" if relative else name
            if wanted[name] == "directory":
                ...
                try:
                    ...
                    if (
                        ...
                        raise PackageDError("captured destination directory differs")
                    visit(child_fd, child)
                finally:
                    os.close(child_fd)
            else:
                ...
                try:
                    ...
                    if (
                        ...
                        raise PackageDError("captured destination file differs")
                    observed = _read_descriptor(
                        file_fd, metadata, label="captured destination file"
                    )
                    if sha256_bytes(observed) != digest:
                        raise PackageDError("captured destination digest differs")
                finally:
                    os.close(file_fd)
...
def _rename_noreplace(
    ...
    if function is None:
        raise PackageDError("atomic no-overwrite finalization is unavailable")
    function.argtypes = [
        ctypes.c_int,
        ctypes.c_char_p,
        ctypes.c_int,
...
def _cleanup_known(
    ...
    try:
        ...
        if _basic_identity(os.fstat(root_fd)) != root_identity:
            raise PackageDError("capture rollback root identity differs")
    ...
    try:
        ...
        os.fsync(parent_fd)
        if failures:
            raise PackageDError("capture rollback encountered an owned-entry failure")
        return
    except OSError as error:
        raise _os_error("verifying capture rollback", error) from error
    raise PackageDError("capture rollback is incomplete; private output was preserved")
...
def capture(source: Path, destination: Path) -> dict[str, Any]:
    ...
    try:
        ...
        if not isinstance(candidate, dict):
            raise PackageDError("candidate record root is not an object")
        ...
        if source_files["candidate.json"] != _stable_identity(candidate_metadata):
            raise PackageDError("candidate record changed after validation")
        ...
        for relative in sorted(records):
            ...
            if artifact_total > caps["total"]:
                raise PackageDError("actual bundle exceeds its role-specific aggregate cap")
        ...
        if observed_files != source_files or observed_directories != source_directories:
            raise PackageDError("source bundle changed during capture")
        if _stable_identity(os.fstat(source_fd)) != source_root_identity:
            raise PackageDError("source bundle root changed during capture")
        ...
        if set(destination_directories) != expected_directories:
            raise PackageDError("destination directory closure differs")
        ...
        if _basic_identity(os.fstat(staging_fd)) != staging_identity:
            raise PackageDError("final destination descriptor identity differs")
        _verify_destination_tree(
            staging_fd,
            expected_destination_files,
            expected_directories,
...
def main() -> int:
    ...
    try:
        ...
        return 0
    except PackageDError as error:
        print(f"Package D capture HOLD: {error}", file=sys.stderr)
        return 3
        ...
        print(f"Package D capture HOLD: invalid private bundle{suffix}", file=sys.stderr)
        return 3
