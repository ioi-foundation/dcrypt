...
- `verify-protocol-specs.py` validates canonical form, schema closure, reviewed constants, the exact subject commit/tree, all 1,395 canonical subject-manifest rows against immutable Git blobs, the separately bound curated classifications, the role-complete critical subset, symlink exclusion, and the artifact manifest without network access or third-party packages. It independently pins reviewed digests for the normative registry, schema, this package guide, authoritative human rendering, and final-rebind tool.
