...
- Do not newly select `2.0.0`; its twelve crates are being yanked from new
  crates.io resolution.
- Do not fall back to `1.2.3`; it contains critical and high-severity defects
  and its twelve crates are also being yanked.
...
The GitHub release object for `v2.0.0` is being removed to prevent it from being
presented as a supported release. The annotated `v2.0.0` Git tag remains
