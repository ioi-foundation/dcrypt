...
pub fn generate_keypair<R: CryptoRng + RngCore>(rng: &mut R) -> Result<(Scalar, Point)> {
    let mut scalar_bytes = Zeroizing::new([0u8; K256_SCALAR_SIZE]);
    loop {
...
pub fn kdf_hkdf_sha256_for_ecdh_kem(
    ikm: &[u8],
    info: Option<&[u8]>,
) -> Result<Zeroizing<[u8; K256_KEM_SHARED_SECRET_KDF_OUTPUT_SIZE]>> {
    ...
    let mut output_array = Zeroizing::new([0u8; K256_KEM_SHARED_SECRET_KDF_OUTPUT_SIZE]);
