//! RFC 9380 BLS12-381 G1 simplified-SWU and 11-isogeny map.
//!
//! The constants are the Montgomery-form values from RFC 9380 Appendix E.2.
//! The formulas follow RFC 9380 Sections 6.6.2 and 8.8.1.
...
fn map_to_isogenous_curve(u: &Fp) -> G1Projective {
    ...
    // Evaluate both SSWU candidates. The inputs are message-derived, but keeping
    // a fixed operation schedule also avoids introducing an unnecessary branch.
    let gx_den_inverse = gx_den.invert().unwrap_or(Fp::zero());
    ...
    let x_num = Fp::conditional_select(&x1_num, &x0_num, gx0_is_square);
    let y0 = y0.unwrap_or(Fp::zero());
    let y1 = y1.unwrap_or(Fp::zero());
    let y = Fp::conditional_select(&y1, &y0, gx0_is_square);
    let y = Fp::conditional_select(&y, &-y, sgn0(&y) ^ sgn0(u));
...
fn isogeny_map(point: &G1Projective) -> G1Projective {
    const COEFFICIENTS: [&[Fp]; 4] = [&ISO11_XNUM, &ISO11_XDEN, &ISO11_YNUM, &ISO11_YDEN];

    let G1Projective { x, y, z } = *point;
    let mut values = [Fp::zero(); 4];

    let mut z_powers = [Fp::zero(); 15];
    z_powers[0] = z;
    for index in 1..z_powers.len() {
        z_powers[index] = z_powers[index - 1] * z;
    }
...
pub(super) fn map_to_curve_g1(field_element: &Fp) -> G1Projective {
