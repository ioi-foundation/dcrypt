...
impl Point {
    ...
    pub fn serialize_uncompressed(&self) -> [u8; P521_POINT_UNCOMPRESSED_SIZE] {
        ...
        result[0] = 0x04;
        self.x.write_bytes(&mut result[1..67]);
        self.y.write_bytes(&mut result[67..133]);
    ...
    pub fn deserialize_uncompressed(bytes: &[u8]) -> Result<Self> {
        ...
        let mut x_bytes = Zeroizing::new([0u8; P521_FIELD_ELEMENT_SIZE]);
        let mut y_bytes = Zeroizing::new([0u8; P521_FIELD_ELEMENT_SIZE]);
    ...
    pub fn deserialize_compressed(bytes: &[u8]) -> Result<Self> {
        ...
        let b = Zeroizing::new(FieldElement::from_bytes(&NIST_P521.b).unwrap());
    ...
    pub fn mul(&self, scalar: &Scalar) -> Result<Self> {
        ...
        let mut r1 = Zeroizing::new(self.to_projective()); // P

        // Montgomery ladder — MSB→LSB
        for byte in scalar.as_secret_buffer().as_ref() {
            for bit in (0..8).rev() {
                let mut choice = Choice::from((byte >> bit) & 1);

                ProjectivePoint::conditional_swap(&mut r0, &mut r1, choice);
                // "ladder step"
                let t0 = Zeroizing::new(r0.add(&r1)); // R0 + R1
                let t1 = Zeroizing::new(r0.double()); // 2·R0
                r1.zeroize();
                *r1 = t0.into_inner();
                r0.zeroize();
                *r0 = t1.into_inner();
    ...
    fn is_on_curve(x: &FieldElement, y: &FieldElement) -> bool {
        ...
        let b_coeff = Zeroizing::new(FieldElement::from_bytes(&NIST_P521.b).unwrap());
...
impl ProjectivePoint {
    ...
    fn conditional_swap(a: &mut Self, b: &mut Self, choice: Choice) {
        ...
        let mut ai = a.is_identity;
        let mut bi = b.is_identity;
    ...
    pub fn add(&self, other: &Self) -> Self {
        ...
        let mut h_is_zero = Choice::from(h.is_zero() as u8);
        let mut r_is_zero = Choice::from(r.is_zero() as u8);
