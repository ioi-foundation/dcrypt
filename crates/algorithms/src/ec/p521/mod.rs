...
pub fn generate_keypair<R: CryptoRng + RngCore>(rng: &mut R) -> Result<(Scalar, Point)> {
    let mut scalar_bytes = Zeroizing::new([0u8; P521_SCALAR_SIZE]);
...
pub fn kdf_hkdf_sha512_for_ecdh_kem(
    ikm: &[u8],
    info: Option<&[u8]>,
) -> Result<Zeroizing<[u8; P521_KEM_SHARED_SECRET_KDF_OUTPUT_SIZE]>> {
    ...
    let mut output_array = Zeroizing::new([0u8; P521_KEM_SHARED_SECRET_KDF_OUTPUT_SIZE]);
