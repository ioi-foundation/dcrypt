...
impl Aes256Gcm {
    /// Generates an AES-256-GCM instance using caller-owned randomness.
    pub fn generate<R: CryptoRng + ?Sized>(rng: &mut R) -> Result<(Self, Aes256Key)> {
...
/// Creates an AES-256-GCM key and nonce from caller-owned randomness and encrypts data.
pub fn aes256_encrypt<R: CryptoRng + ?Sized>(
    rng: &mut R,
    plaintext: &[u8],
    aad: Option<&[u8]>,
...
pub fn aes256_encrypt_package<R: CryptoRng + ?Sized>(
    rng: &mut R,
    plaintext: &[u8],
    aad: Option<&[u8]>,
