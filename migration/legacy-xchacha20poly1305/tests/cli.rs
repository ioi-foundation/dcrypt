...
const ACKNOWLEDGEMENT: &str = "dcrypt-v0.7.0-pre-through-v1.2.3-custom-xchacha20poly1305";
...
fn malformed_legacy_serializations_are_rejected_before_output_creation() {
    ...
    let input = directory.0.join("ciphertext.bin");
    let output = directory.0.join("plaintext.bin");
    fs::write(&key, SERIALIZED_KEY.replacen("dcrypt", "DCRYPT", 1)).unwrap();
    fs::write(&input, CIPHERTEXT).unwrap();
    ...
    fs::write(&key, SERIALIZED_KEY).unwrap();
    let result = binary()
        .args([
            "--acknowledge-format",
            ACKNOWLEDGEMENT,
...
fn duplicate_and_mixed_format_options_are_rejected() {
    ...
    fs::write(&serialized, SERIALIZED_KEY).unwrap();
